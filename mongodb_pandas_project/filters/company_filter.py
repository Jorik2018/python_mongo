...
def filter():
    ...
    filtered_path = f'{backup_dir}/company_filtered.csv'
    nrows = 1000
    if os.path.exists(filtered_path):os.remove(filtered_path)
    while True:
        df = pd.read_csv(file_path,skiprows = range(1, 000), nrows = nrows)
        if df.empty:
            if(file_exists(filtered_path)):upload(filtered_path,f'company_filtered_{get_timestamp()}.csv' )
            break
        print(df)
